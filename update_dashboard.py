        ...
        <div id="lookup" class="tab-content">
            <div class="lookup-bar">
                <div class="mode-toggle">
                    <button class="mode-btn active" data-mode="single">1 ngày</button>
                    <button class="mode-btn" data-mode="range">Khoảng ngày</button>
                </div>
                ...
                <div class="channel-pills">
                    <button class="ch-pill active" data-channel="all">Tất cả</button>
                    <button class="ch-pill" data-channel="shopee">Shopee</button>
                    <button class="ch-pill" data-channel="tiktok">TikTok</button>
                    <button class="ch-pill" data-channel="web">Web</button>
                    <button class="ch-pill" data-channel="lazada">Lazada</button>
                </div>
...
function lookup(){{
    ...
    // Re-render SP detail neu da chon SP truoc do
...
function aggregateSP(spName,start,end){{
...
function renderSPDetail(spName){{
    selectedSP=spName;
    const [s,e]=getLookupRange();
    const data=aggregateSP(spName,s,e);
    if(!data){{
...
document.querySelectorAll("#lookup .mode-btn").forEach(b=>{{b.addEventListener("click",e=>{{
    lookupMode=e.target.dataset.mode;
    document.querySelectorAll("#lookup .mode-btn").forEach(t=>t.classList.remove("active"));
    e.target.classList.add("active");
...
document.querySelectorAll("#lookup .ch-pill").forEach(b=>{{b.addEventListener("click",e=>{{
    lookupChannel=e.target.dataset.channel;
    document.querySelectorAll("#lookup .ch-pill").forEach(t=>t.classList.remove("active"));
    e.target.classList.add("active");
    lookup();
}});}});
